...
contract('CryptoPunksMarket-buySellRemoveFromSale', function (accounts) {
  it("can not offer for sale allPunksAssigned = false", async function () {
    ...
    //allAssigned is false so offerPunkForSale will be revert
    await expectThrow(contract.offerPunkForSale(0, 1000));
  }),
    it("can offer a punk", async function () {
      var contract = await CryptoPunksMarket.deployed();
      console.log("allPunksAssigned:", await contract.allPunksAssigned.call())
    ...
    it("can not buy a punk that is not for sale", async function () {
      //deploy new version 
      var contract = await CryptoPunksMarket.new({ from: accounts[0] });
      console.log("allPunksAssigned:", await contract.allPunksAssigned.call())
      await contract.setInitialOwner(accounts[0], 1);
      await contract.allInitialOwnersAssigned();
      await contract.punkNoLongerForSale(1);
      //punk1 is not for sale so buyPunk will be revert
    ...
    it("can not buy a punk for too little money", async function () {
      ...
      assert(ethBalance > 0);
      var offer=await contract.punksOfferedForSale.call(0)
     //min value set=1000 from test case above so buyPunk will revert
    ...
    it("can withdraw money from sale", async function () {
      ...
      await contract.withdraw();//transfer money to msg.sender== accounts[0]
    ...
    it("can offer for sale then cancel it", async function () {
      var contract = await CryptoPunksMarket.deployed();
